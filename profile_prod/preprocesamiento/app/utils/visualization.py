...
class ProfileVisualizationManager:
    """Visualization utilities for profile preprocessing service"""
    ...
    def create_detection_grid(self, original_image: np.ndarray,
        ...
        """
        Create a grid visualization showing original image and cropped faces
        ...
        grid_rows = (num_faces + grid_cols - 1) // grid_cols + 1  # +1 for original image
        ...
        # Place cropped faces
        for i, (cropped_face, detection) in enumerate(zip(cropped_faces, detections)):
            ...
            # Resize cropped face to cell size
            face_resized = cv2.resize(cropped_face, (cell_width, cell_height))
            ...
            # Place in grid
            grid_image[start_row:end_row, start_col:end_col] = face_resized
            
            # Add title
            title = f"Face {i+1} (conf: {detection['confidence']:.2f})"
    ...
    def create_processing_summary(self, processing_result: Dict) -> np.ndarray:
        ...
        # Title
        title = "Profile Preprocessing Summary"
        ...
        stats = [
            ...
            f"Processed Faces: {len(processing_result['processed_faces'])}",
        ...
        # Draw face details if available
        if processing_result['processed_faces']:
            y += 20
            cv2.putText(canvas, "Face Details:", (20, y),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
            y += 30
            
            for i, face in enumerate(processing_result['processed_faces']):
                face_info = f"Face {i+1}: {face['class_name']}, conf: {face['confidence']:.3f}"
                cv2.putText(canvas, face_info, (40, y),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 1)
                y += 25
    ...
    def create_complete_visualization(self, original_image: np.ndarray,
        ...
        try:
            # Extract cropped faces from base64
            cropped_faces = []
            for face_data in processing_result['processed_faces']:
                # Decode base64 to get cropped face
                face_base64 = face_data['cropped_image_base64']
                face_bytes = base64.b64decode(face_base64)
                pil_image = Image.open(io.BytesIO(face_bytes))
                face_array = np.array(pil_image.convert('RGB'))
            ...
            if cropped_faces:
                detection_grid = self.create_detection_grid(
                    original_image, 
                    cropped_faces, 
                    processing_result['processed_faces']
                )
    ...
    def create_side_by_side_comparison(self, original_image: np.ndarray,
        ...
        Args:
            original_image: Original input image
            processed_faces: List of processed face images
        ...
        num_images = len(processed_faces) + 1  # +1 for original
        ...
        # Place processed faces
        for i, face in enumerate(processed_faces):
            start_col = (i + 1) * cell_width
            end_col = start_col + cell_width
            
            # Resize face to fit cell
            face_resized = cv2.resize(face, (cell_width, canvas_height))
            canvas[:, start_col:end_col] = face_resized
            
            # Add title
            title = titles[i + 1] if titles and len(titles) > i + 1 else f"Face {i + 1}"
