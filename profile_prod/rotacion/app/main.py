...
from app.models.profile_rotation_pipeline import ProfileRotationPipeline
from app.utils.image_processing import ImageProcessor
from app.utils.visualization import ProfileRotationVisualizer
...
app = FastAPI(
    title="Profile Rotation Assessment API",
    description="Advanced profile rotation classification to assess viability for anthropometric and morphological analysis",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
def _load_pipeline():
    """Lazy load profile rotation pipeline"""
    model_path = MODELS_DIR / "best_profile_classifier_multilabel.pth"
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model file not found at {model_path}. Please place the trained model in the /models directory."
        )
    try:
        pipeline = ProfileRotationPipeline(str(model_path))
        logger.info("✅ Profile rotation pipeline loaded successfully!")
        return pipeline
...
def _load_visualizer():
    ...
    return ProfileRotationVisualizer()

def get_pipeline():
    """Get profile rotation pipeline, loading it if necessary"""
...
async def startup_event():
    ...
    logger.info("🚀 Initializing Profile Rotation API with lazy loading...")
...
@app.post("/analyze-profile-rotation")
async def analyze_profile_rotation(
    file: UploadFile = File(...),
    confidence_threshold: float = Form(0.5),
    include_visualization: bool = Form(True),
    enhance_image: bool = Form(True),
    pipeline: ProfileRotationPipeline = Depends(get_pipeline),
    processor: ImageProcessor = Depends(get_image_processor),
    visualizer: ProfileRotationVisualizer = Depends(get_visualizer)
):
    """
    Complete profile rotation analysis to assess viability for anthropometric and morphological analysis
    ...
    try:
        ...
        results = pipeline.analyze_profile_rotation(processed_image, confidence_threshold)
        ...
        if include_visualization and results['analysis_summary']['processing_successful']:
            try:
                viz_filename = f"rotation_analysis_{results['analysis_id']}.png"
        ...
        logger.info(f"Profile rotation analysis completed - ID: {results['analysis_id']}, "
...
async def classify_rotation(
    ...
    try:
        ...
        # Log results
        logger.info(f"Rotation classification completed - ID: {results['analysis_id']}, "
...
async def assess_viability(
    ...
    """
    Assess profile viability for anthropometric and morphological analysis
    ...
    try:
        ...
        # Log results
        logger.info(f"Viability assessment completed - ID: {results['analysis_id']}, "
...
async def assess_image_quality(
    ...
    try:
        ...
        # Log results
        logger.info(f"Image quality assessment completed - ID: {analysis_id}, "
...
@app.get("/model-info")
async def get_model_info(pipeline: ProfileRotationPipeline = Depends(get_pipeline)):
    """Get information about the loaded rotation model"""
    return {
        ...
        "model_architecture": "EfficientNet-B0-MultiLabel-Profile-Rotation"
    }
...
async def batch_analyze_rotation(
    ...
    """
    Batch process multiple profile images for rotation analysis
    
    Args:
        files: List of profile image files
        confidence_threshold: Minimum confidence threshold (0.0-1.0)
    ...
    try:
        ...
        logger.info(f"Batch rotation analysis completed - ID: {batch_id}, "
        ...
        logger.error(f"Error in batch rotation analysis: {e}")
