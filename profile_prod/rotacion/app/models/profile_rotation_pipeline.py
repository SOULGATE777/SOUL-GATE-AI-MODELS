...
class ProfileMultiLabelRotationClassifier(nn.Module):
    """
    Multi-label CNN model for profile rotation classification
    Based on EfficientNet-B0 architecture
    """
    def __init__(self, num_classes):
        super(ProfileMultiLabelRotationClassifier, self).__init__()
        ...
        # Multi-label classifier for profile rotations
...
class ProfileRotationPipeline:
    """
    Profile rotation assessment pipeline for determining if profile images
    are suitable for anthropometric and morphological analysis.
    
    Uses a multi-label CNN to classify profile orientation and quality.
    """
    
    def __init__(self, model_path: str, device: str = 'auto'):
        """
        Initialize the profile rotation pipeline
        ...
        self.class_names = checkpoint.get('class_names', [
            'aceptable', 'hacia_abajo', 'hacia_arriba',
            'desde_abajo_o_mandibula_hacia_primer_plano',
            'desde_arriba_o_superior_hacia_primer_plano',
            'rotado_lejos_o_desde_atras',
            'rotado_hacia_desde_adelante'
        ])
        ...
        # Initialize model
        self.model = ProfileMultiLabelRotationClassifier(self.num_classes)
        ...
        logger.info(f"Profile rotation pipeline initialized successfully")
    ...
    def predict_rotation_pattern_aware(self, image: np.ndarray, threshold: float = None) -> Dict[str, Any]:
        """
        Pattern-aware multi-label prediction for profile rotation assessment
        ...
        try:
            ...
            # Apply pattern constraints for profiles
            ...
            if aceptable_pred:
                # If aceptable is predicted, profile is suitable for analysis
                ...
                is_suitable = False
                if rotation_issues:
                    suitability_reason = f"Profile has rotation issues: {', '.join(rotation_issues)}"
                else:
                    suitability_reason = "Profile quality assessment unclear - low confidence predictions"
            ...
            logger.error(f"Error in rotation prediction: {e}")
            raise
    ...
    def analyze_profile_rotation(self, image: np.ndarray, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        """
        Complete profile rotation analysis with detailed assessment
        ...
        try:
            ...
            logger.info(f"Profile rotation analysis completed - ID: {analysis_id}, "
            ...
            logger.error(f"Error in profile rotation analysis: {e}")
            return {
                'analysis_id': analysis_id,
                'analysis_summary': {
    ...
    def _generate_recommendation(self, rotation_results: Dict[str, Any]) -> str:
        """
        Generate actionable recommendations based on rotation assessment
        ...
        if rotation_results['is_suitable']:
            return "Profile orientation is acceptable. Proceed with anthropometric and morphological analysis."
        ...
        if rotation_results['rotation_issues']:
            ...
            for issue in issues:
                if 'hacia_abajo' in issue:
                    recommendations.append("Adjust camera angle upward to capture profile at eye level")
                elif 'hacia_arriba' in issue:
                    recommendations.append("Lower camera angle to capture profile at eye level")
                elif 'desde_abajo' in issue:
                    recommendations.append("Raise camera position to avoid upward angle distortion")
                elif 'desde_arriba' in issue:
                    recommendations.append("Lower camera position to avoid downward angle distortion")
                elif 'rotado_lejos' in issue or 'desde_atras' in issue:
                    recommendations.append("Rotate subject toward camera for better profile visibility")
                elif 'rotado_hacia' in issue or 'desde_adelante' in issue:
                    recommendations.append("Adjust subject position for true profile view (90-degree angle)")
            
            if recommendations:
                return f"Rotation issues detected. Recommendations: {'; '.join(recommendations)}"
        
        return "Profile orientation unclear. Ensure proper lighting and positioning for clear profile capture."
    ...
    def classify_rotation_only(self, image: np.ndarray, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error in rotation classification: {e}")
            return {
                'analysis_id': analysis_id,
                'processing_successful': False,
                'error': str(e)
    ...
    def assess_viability_only(self, image: np.ndarray, confidence_threshold: float = 0.5) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error in viability assessment: {e}")
            return {
                'analysis_id': analysis_id,
                'processing_successful': False,
                'error': str(e)
